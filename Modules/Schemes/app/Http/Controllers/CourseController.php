...
class CourseController extends Controller
{
    ...
    public function generateEnrollmentKey(Course $course)
    {
        ...
        $user = auth('api')->user();
        if (! \Illuminate\Support\Facades\Gate::forUser($user)->allows('update', $course)) {
            return $this->error('Anda tidak memiliki akses untuk generate enrollment key course ini.', 403);
        }
    ...
    public function removeEnrollmentKey(Course $course)
    {
        ...
        return $this->success(['course' => $updated], 'Enrollment key berhasil dihapus dan enrollment type diubah ke auto_accept.');
    }
