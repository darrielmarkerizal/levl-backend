...
class EnrollmentKeyTest extends TestCase
{
    ...
    public function admin_can_generate_enrollment_key()
    {
        $response = $this->actingAs($this->admin, 'api')
            ->postJson("/api/v1/courses/{$this->course->slug}/enrollment-key/generate");
        ...
        $response->assertStatus(200)
            ->assertJsonStructure([
                ...
                'data' => [
                    'enrollment_key',
                    'course' => ['id', 'slug', 'title', 'enrollment_key'],
                ],
            ]);
    ...
    {
        $response = $this->actingAs($this->admin, 'api')
            ->putJson("/api/v1/courses/{$this->course->slug}/enrollment-key", [
    ...
    public function admin_can_remove_enrollment_key()
    {
        ...
        $response = $this->actingAs($this->admin, 'api')
            ->deleteJson("/api/v1/courses/{$this->course->slug}/enrollment-key");
